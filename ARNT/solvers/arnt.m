...
if ~isfield(opts, 'switch');         opts.switch = true; end
if ~isfield(opts, 'maxit_sw');       opts.maxit_sw = 400; end
if ~isfield(opts, 'do_init');        opts.do_init = true; end
if ~isfield(opts, 'zeta');           opts.zeta = 0.1; end
...
if isfield(opts, 'record_fid')
        fid = opts.record_fid; hasRecordFile = 1;
end
...
if ~isfield(opts,'solver_init')
    solver_init = [];
else
    solver_init = opts.solver_init;
end

if ~isempty(solver_init) && opts.do_init
    if strcmp(func2str(solver_init), 'RGBB') || strcmp(func2str(solver_init), 'RAdaGBB')
        if opts_init.record
            fprintf('initial solver\n');
        end
        t1 = tic; [x,~, outin] = feval(solver_init, x, fun, M, opts_init, varargin{:}); t2 = toc(t1);
    else
...
if ~isfield(opts,'opts_sub')
    ...
    opts_sub = opts.opts_sub;
end

if hasRecordFile
    opts_sub.record_fid = opts.record_fid;
end
stagnate_check = 0;
...
for iter = 1:maxit
    ...
    if nrmG >= 1
        opts_sub.maxit = opts_sub.maxitvec(1);
    ...
    % criterion of PCG
    opts_sub.gtol = max(min(opts.zeta*nrmG, 0.1), 1*gtol); % tol for subproblem
    ...
    switch out.flag(iter)
        ...
        case{2}
            stepinf = '\t[Exceed maxit]\n';
            maxnct = mod(maxnct + 1,3);
        case{1}
            stepinf = '\t \n';
    end
    ...
    % ---- termination ----
    if (nrmG <= gtol && iter>0)  %|| ( (FDiff <= ftol) && ratio > 0 )
        out.msg = 'opt';
        ...
        if sum(out.iter_sub(iter-2: iter)) <= 6
            if ~isempty(solver_init)
                ...
                if strcmp(func2str(solver_init), 'RGBB') || strcmp(func2str(solver_init), 'RAdaGBB')
                    ...
                    [x,~, out2] = feval(solver_init, x, fun, M, opts,varargin{:});
                else
                    [x,~, out2] = feval(solver_init, x, fun, opts,varargin{:});
                end
                t_local = toc(t_local);
...
out.avginit = sum(out.iter_sub);
out.iter = length(out.iter_sub);
out.avginit = ceil(sum(out.iter_sub)/out.iter);
out.time = timetoc;
out.time_ARNT = t_arnt;
