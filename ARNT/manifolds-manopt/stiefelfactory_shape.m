function M = stiefelfactory_shape(d, m, k, gpuflag)
...
%     end
    ...
    function rhess = ehess2rhess(X, egrad, ehess, H)
        X = vecX(X); egrad = vecX(egrad); ehess = vecX(ehess); H = vecX(H);
        XtG = multiprod(multitransp(X), egrad);
        symXtG = multisym(XtG);
        HsymXtG = multiprod(H, symXtG);
        rhess = projection(reshapeX(X), reshapeX(ehess - HsymXtG));
        %rhess = reshapeX(rhess);
    end
...
%             Y(:, :, kk) = Y(:, :, kk)*(u.*s'*v');
    ...
    M.rand = @(X) reshapeX(qr_unique(vecX(X)));
    ...
    % reshape X from d * m^2 to dm * m
    function vecX = vecX(X)
%         vecX = zeros(n,p);
%         for i=1:p
%             vecX(:,i) = reshape(X(:,(i-1)*p+1:i*p),n,1);
%         end
        vecX = reshape(X,n,p);
    end

 % reshape X from dm * m to d * m^2
    function matX = reshapeX(X)
%         matX = zeros(d,m^2);
%         for i=1:p
%             matX(:,(i-1)*p+1:i*p) = reshape(X(:,i),d,m);
%         end
        matX = reshape(X,d,m^2);
    end
