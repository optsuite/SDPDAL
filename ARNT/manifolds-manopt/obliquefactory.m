function M = obliquefactory(n, m, scale, transposed)
...
%       computations as part of the log function.
    ...
    M.proj = @(X, U) trnsp(projection(trnsp(X), trnsp(U), scale));
    ...
    function rhess = ehess2rhess(X, egrad, ehess, U)
        ...
        PXehess = projection(X, ehess, scale);
        inners = sum(X.*egrad, 1);
        rhess = PXehess - bsxfun(@times, U, inners)/scale^2;
    ...
    M.retr = @(x, d, t) retraction(x, d, t, scale);
    % Retraction on the oblique manifold
    function y = retraction(x, d, t, scale)
        x = trnsp(x);
        d = trnsp(d);
...
function PXH = projection(X, H, scale)
    ...
    % the root points X(:, i).
    PXH = H - bsxfun(@times, X, inners)/scale^2;
