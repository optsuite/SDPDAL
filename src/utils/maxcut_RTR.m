function [x, cutvalue, cutvalue_upperbound, Y, info] = maxcut_RTR(L, r, opts)
...
%       more reliable and more flexible than using a global variable.
    ...
    for rr = r
        ...
        [Y, info] = maxcut_fixedrank(L, Y0, opts);
...
function [Y, info] = maxcut_fixedrank(L, Y, opts)
...
% Laplacian of the graph L and an initial guess Y. L is nxn and Y is nxr.
    ...
    problem.egrad = @egrad;    M.proj = @(X, U) projection(X, U);
